...
const AI_MODEL = "deepseek-chat"; // V4 Flash — non-deprecated as of July 2026
...
async function callDeepSeek(system: string, prompt: string): Promise<string> {
  ...
  try {
    ...
    if (!res.ok) return prompt; // fallback to template
    const data = (await res.json()) as {
      choices?: Array<{ message?: { content?: string } }>;
    };
    return data.choices?.[0]?.message?.content ?? prompt;
  } catch {
    return prompt; // network error — fallback to template
  }
}
