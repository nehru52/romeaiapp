...
import { contentReverseEngineer } from "../services/content-reverse-engineer.js";
import { contentService } from "../services/content-service.js";
import { PackService } from "../services/pack-service.js";
import { promptCache } from "../services/prompt-cache.js";
import { getTelegramBot, linkTenantChat } from "../services/telegram-bot.js";
...
async function hashPassword(pw: string): Promise<string> {
  try {
    return await (Bun as any).password.hash(pw);
  } catch {
    return `hashed:${pw}`;
  }
}
async function verifyPassword(pw: string, hash: string): Promise<boolean> {
  try {
    return await (Bun as any).password.verify(pw, hash);
  } catch {
...
app.post("/api/onboarding/website", async (c) => {
  ...
  try {
    // Use real Firecrawl scraper if configured, fall back to mock analysis
    let analysis;
    if (websiteScraper.isConfigured()) {
      analysis = await websiteScraper.analyze(url);
    } else {
      // Fallback to the workflow's mock analysis
      const result = await workflow.submitWebsite(userId, url);
      analysis = result.analysis;
    }

    // Create tenant using scraped data
    const tenant = workflow.tenants.createTenant({
      name: analysis.title ?? url.replace(/https?:\/\//, "").split("/")[0],
      slug:
        analysis.title?.toLowerCase().replace(/[^a-z0-9]+/g, "-") ?? "business",
