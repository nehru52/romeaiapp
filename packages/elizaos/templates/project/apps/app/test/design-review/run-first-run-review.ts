/**
 * First-run E2E screenshot runner.
 *
 * Walks through every first-run step, screenshots each one at multiple
 * viewports, then generates an HTML gallery alongside the design-review output.
 *
 * Usage:
 *   npx tsx apps/app/test/design-review/run-first-run-review.ts
 *   ELIZA_DESIGN_REVIEW_HEADLESS=1 npx tsx apps/app/test/design-review/run-first-run-review.ts
 */
...
const outputRoot = path.resolve(
  appRoot,
  "test-results/design-review/first-run",
);
...
const FIRST_RUN_PERMISSION_FLOW =
...
const steps: StepSpec[] = [
  ...
  },
  {
    ...
    setup: async (page) => {
      // Use the stable offline path so the full first-run flow stays local.
  ...
  },
  {
    ...
    setup: async (page) => {
      ...
      if (FIRST_RUN_PERMISSION_FLOW === "grant") {
...
async function createPage(
  ...
  // Pre-initialize storage so the app loads fresh into first-run
  ...
  // Intercept /api/config to return an empty config so first-run doesn't
  ...
  // Always report first-run as not complete so each viewport run starts fresh.
  await page.route("**/api/first-run/status", (route) => {
    return route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  // Swallow the first-run submit POST so the mock doesn't flip its state.
...
<html lang="en">
  <head>
    ...
    <title>Eliza First-run Review</title>
    <style>
...
async function captureFirstRunFlow(
  browser: Browser,
  appBaseUrl: string,
  apiBaseUrl: string,
  viewport: ViewportSpec,
  ...
  try {
    // Navigate to app root — should show first-run since mock API has firstRunComplete=false
    ...
    // Wait for loading screen to disappear and the first first-run screen to appear.
...
async function main(): Promise<void> {
  console.log("First-run Review — starting...\n");

  await rm(outputRoot, { recursive: true, force: true });
  await mkdir(screenshotsRoot, { recursive: true });

  // Start mock API with first-run NOT complete so we see the first-run flow
  const api = await startMockApiServer({ firstRunComplete: false, port: 0 });
  ...
  try {
    for (const viewport of viewports) {
      ...
      const result = await captureFirstRunFlow(
        browser,
        appBaseUrl,
        api.baseUrl,
        viewport,
