...
export class Editor implements Component, Focusable {
  ...
  private getLine(index: number): string {
    return this.state.lines[index] as string;
  }

  /** Get current line at cursor */
  private get currentLine(): string {
    return this.state.lines[this.state.cursorLine] as string;
  }
  ...
  private navigateHistory(direction: 1 | -1): void {
    ...
    if (this.historyIndex === -1) {
      ...
      this.setTextInternal(this.history[this.historyIndex] as string);
    }
  }
  ...
  private insertTextAtCursorInternal(text: string): void {
    ...
    if (insertedLines.length === 1) {
      ...
      this.setCursorCol(
        (insertedLines[insertedLines.length - 1] as string).length,
      );
    }
  ...
  private yank(): void {
    ...
    const text = this.killRing[this.killRing.length - 1] as string;
  ...
  private yankPop(): void {
    ...
    const lastEntry = this.killRing.pop() as string;
  ...
  private insertYankedText(text: string): void {
    ...
    if (lines.length === 1) {
      ...
      this.state.lines[this.state.cursorLine] = before + (lines[0] as string);
  ...
  private deleteYankedText(): void {
    if (this.killRing.length === 0) return;
    const yankedText = this.killRing[this.killRing.length - 1] as string;
    ...
    if (yankLines.length === 1) {
      ...
      const startCol =
        startLineContent.length - (yankLines[0] as string).length;
  ...
  private undo(): void {
    ...
    const snapshot = this.undoStack.pop() as EditorState;
