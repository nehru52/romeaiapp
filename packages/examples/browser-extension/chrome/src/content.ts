...
function extractPageContent(): ExtractedContent {
  ...
  console.log(
    "[Content Script] Found main element:",
    mainElement?.tagName,
  ...
  console.log(
    "[Content Script] Visible elements:",
    uniqueVisible.length,
    "Visible text length:",
    visibleText.length,
  );
...
function cropToTokenLimit(
  ...
  const endIndex = Math.min(
    fullText.length,
    visibleIndex + visibleText.length + paddingChars,
  );
  ...
  cropped += "\n--- CURRENTLY VISIBLE ON SCREEN START ---\n";
  cropped += fullText.substring(
    visibleIndex,
    Math.min(visibleIndex + visibleText.length, endIndex),
  );
  cropped += "\n--- CURRENTLY VISIBLE ON SCREEN END ---\n";
  ...
  try {
    switch (request.type) {
      ...
      case "CAPTURE_VISIBLE_TAB": {
        ...
        sendResponse({
          type: "ERROR",
          error: "Use background script for screenshots",
        });
        break;
      }
    }
