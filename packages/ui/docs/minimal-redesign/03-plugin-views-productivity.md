...
1. **Two competing visual systems, neither light.**
   ...
   - Calendar uses `bg-bg/20` / `bg-white/3` / `ring-white/50` dark tints. Feed uses inline `game-surface-shell` primitives with a dark `rgba(0,0,0,0.78)` image hero.
...
2. **Three off-brand blue terminal twins** ship inside the comms files: `ContactsTuiView`, `PhoneTuiView`, `MessagesTuiView` (~1300 lines total) in `#020617` bg + `#7dd3fc` blue. Companion (`CompanionTuiView`) is a 4th. These directly violate "no blue / lighter / single look." They are ~50% of each comms file.
...
- **Real or stub?** **Real.** `@elizaos/capacitor-contacts`: `requestPermissions`/`listContacts`/`createContact`/`importVCard` (`:116-124,178,206`); non-native short-circuits to empty (`:102`). TUI twin via `loadContactsState()` (`:1019`).
- **States**: `Mode = "list"|"detail"|"new"` (`:53,95`). list: loading (`:406`), first-run empty w/ SVG motif + import CTA (`:414`), no-match (`:437`), populated `<ul>` (`:447`). detail read-only panel (`:603`). new create-form (`:753`). error banner any-mode (`:350`). Separate `ContactsTuiView` (`:1002-1455`).
- **Visual structure**: 1 sticky header w/ back + dynamic `<h1>` (`:270`); empty-state body paragraph (`:426); read-only explainer note (`:654`) "Editing existing contacts is unavailable on this device."; detail field-groups are bordered cards (`:736` `"rounded-xl border border-border/30 bg-bg-accent/40 px-4 py-3"`); ~5 border families (header `:270`, search `:330`, list `divide-y` `:448`); starred `Star` badge (`:584,617`); ~8+ buttons; search + 3 create inputs (`:822,845,866`) + hidden file input. **56-line hand-drawn `AddressBookMotif` SVG** (`:462-519`) for one empty state. TUI half: 2 blue-bordered sections + duplicated create form + vCard textarea (`#020617`/`#7dd3fc`).
- **Heaviness/slop**: 1455 lines for a read-only phone book; ~450 are the blue TUI twin. Decorative `AddressBookMotif` slop. Detail wraps phones/emails in bordered uppercase-labeled cards (`ContactFieldGroup` `:724`). Apologetic read-only note (`:654`). 10+ verbose `useAgentElement` `description` strings read like prose comments.
- **Minimize**: Delete the TUI twin (`:935-1455`) or force single light theme (can't stay blue). Delete `AddressBookMotif` (`:462-519`) — empty = one `Users` icon + one line + import action. Flatten detail: drop `ContactFieldGroup` cards/borders/uppercase labels; render avatar + name + flat icon+value rows (Phone/Mail icon = label) + inline call/text icons. Remove read-only note. Starred = orange `Star` (today amber `:586`); call = orange glyph, text = blue `#1d91e8`. New/import as the 2 agent-exposed actions; replace the 3-field form (`:753-904`) with chat intent ("add Jane, 555-1234").
...
- **Real or stub?** **Real.** `@elizaos/capacitor-phone`: `requestPermissions`/`listRecentCalls`/`placeCall` (`:358-366,424`); TUI adds `openDialer`/`saveCallTranscript`/`getStatus`. Cross-view seed `consumePendingPhoneNumber()` (`:380`).
- **States**: `PhoneTab = "dialer"|"recent"` (`:61,337`). dialer: `<output>` display (`:623`), inline call-error (`:643`), 12-key pad + +/call/backspace. recent: error (`:724`), loading (`:729`), empty w/ icon + 2 CTAs (`:736`), populated `<ul>` (`:775`). Lazy-load guard (`:349,389`). Separate `PhoneTuiView` (`:793-1281`) adds a **transcript-save form** not in GUI.
- **Visual structure**: header `<h1>Phone</h1>` + **subtitle** "Dialer and recent calls" (`:553`, redundant w/ tabs below); recent-empty paragraph (`:745`, long/apologetic permission explainer); recent rows = surfaced buttons (`:211`); 12 bordered dial keys (`:170`); call-type icon chips (`:218`); **~22 buttons**; 0 GUI inputs (TUI adds 4). Evidence: shell `:533`; header `:536`; dial key `:170` w/ inline border style; `defaultOverlayContext()` hardcodes `uiTheme:"light"` (`:68`). TUI: 2 blue sections + full dialpad re-impl + transcript form (`:1123-1225`).
- **Heaviness/slop**: 1281 lines for a dialer (~3× too heavy, half from blue TUI). Subtitle "Dialer and recent calls" duplicates the two tabs. Empty state = icon + heading + permission paragraph + 2 redundant buttons. Inline `onMouseEnter`/`Leave` hover hack on call button (`:685-690`). TUI transcript form (`:1123`) is a feature that exists ONLY in the terminal twin — dead weight or missing GUI capability, either way inconsistent.
- **Minimize**: Delete subtitle (`:553`). Collapse empty state to faint icon + "No recent calls."; permission as one inline pill only when denied; drop redundant Dialer/Refresh buttons. Call-type = color only (outgoing orange, incoming green, missed red; no text label). Borderless light dialpad, orange call button only; replace inline hover hack with design-system hover. Remove `uiTheme:"light"` plumbing + blue TUI. Proactive: foreground who-to-call-now (missed call to return / scheduled call); push keypad behind one tap; "call Mom" via chat is primary.
- **Even-simpler**: Dialpad is the only unique thing and the least voice-forward element. Recent-calls is just a filtered comms log. **Fold into unified Comms** as a "calls" filter + behind-one-tap keypad. TUI transcript form should not survive.
...
- **Even-simpler**: **Natural anchor for a single unified Comms surface.** Threads/calls/contacts are the same primitive (person + history + action). A merged Comms view = one searchable list of people/threads (SMS + calls interleaved) → tap → timeline w/ inline call/text/email; all composing/dialing/lookup pushed into chat. **Would replace all 3 comms files + their 3 TUI twins, eliminating ~3900 lines.** Contacts = "people" filter, Phone = "calls" filter + behind-one-tap keypad, Messages = default "threads" view.
...
- **States**: scene loading vs `avatarReady` (`:22`, `useCompanionSceneStatus`); emote picker open/closed (`EmotePicker`). The TUI twin (`CompanionTuiView` `:182-372`) is a separate dev/state surface.
- **Visual structure**: avatar scene fills the view (`pointerEvents:"none"` overlay). One top-left **status chip cluster** (`:41-72`): `StatusChip` (ready/loading) + 3 `CompanionChip`s ("N emotes", "N/M catalog", "overlay relay") in a blurred translucent pill. Evidence: chip cluster `:41-59` (`border:"1px solid var(--border)"`, `backdropFilter:"blur(12px)"`, `boxShadow`). TUI half (`:267-372`): `#020617` bg + `#7dd3fc` blue, two bordered "state"/"controls" sections dumping message counts/cloud flags/emote categories + 4 buttons.
- **Heaviness/slop**: The GUI view is already very light and on-direction (avatar + chat in the floating pill — exactly the redesign). The only slop is the **top-left status chip cluster** (`:41-72`): "N emotes / N/M catalog / overlay relay" are devtools-flavored info nobody needs at a glance. The `CompanionTuiView` is a 4th off-brand blue terminal twin (~190 lines).
- **Minimize**: Keep the bare avatar + floating-pill chat (the model for the whole redesign). Reduce the status cluster to a single small `ready`/`loading` dot (orange loading → success when ready); drop "N emotes / catalog / overlay relay" chips (`:63-71`). Emote picker is fine as the one view-dependent action. Kill the blue TUI palette.
...
- **plugin / view id / file**: `@elizaos/plugin-feed` / `feed` (icon `Gamepad2`, "Feed prediction market operator dashboard") / `plugins/plugin-feed/src/ui/FeedOperatorSurface.tsx` (644 L) + `game-surface-shell.tsx`. Decl: `plugins/plugin-feed/src/index.ts:6`.
- **Purpose**: Operator dashboard to spectate-and-steer an autonomous agent trading the Feed prediction-market game.
- **Real or stub?** **Real, most data-dense.** `loadDashboard` fires **10 parallel client calls** (`:213-235`): status/summary/goals/trades/markets/team dashboard/team conversations/chat/wallet/balance; polls 12s (`:272`). Toggle-autonomy + suggested-prompt buttons agent-instrumented (`:197,113`) → `controlAppRun`/`sendAppRunMessage`. Separate `FeedTuiView` (`:625`).
...
- **Visual structure**: **34vh background-image `GameSurfaceHero`** w/ gradient overlay + title + status pill + CTA (`game-surface-shell.tsx:39-140`, invoked `:391`); `WaitingForSession` message (`:343`) + statusMessage box (`:611`) + card subtitles; `GameSurfaceStrip` of 4 stat chips (`:404`) + up to **4 `SurfaceSection`s** each w/ multiple `SurfaceCard`s (~12+ cards); heavy borders (hero `borderBottom`, strip border, each chip bordered, `rounded-xl border border-border/30 bg-bg/60` `:533,549`, status box `rounded-2xl border border-border/35 bg-card/70` `:612`); **3 `SurfaceBadge`s** (status/viewerAttachment/health `:407`) + "N active runs" (`:416`); Pause/Resume CTA (`:396`) + steering button (`:595`) + 2 suggested-prompt buttons (`:561`); 0 inputs (steering via preset buttons). Evidence: hero `game-surface-shell.tsx:60-65` `height:"34vh"` + `linear-gradient(...rgba(0,0,0,0.78))` overlay `:71`.
...
- **Even-simpler**: **FLAG — does not belong in a productivity/lifeops assistant.** Crypto prediction-market GAME operator surface (autonomous trading/PnL/wallets, `Gamepad2`). Shares `game-surface-shell.tsx` w/ other games; registers via `registerOperatorSurface`. Almost entirely chat-replaceable ("how's my Feed agent / pause it / what markets are live"). Recommend excluding it (and the whole game-operator surface family) from the default lifeops view set; keep only for users who install the Feed game.
...
4. **Comms trio (Contacts/Phone/Messages)** — 1455/1281/1202 lines, each with a **blue TUI twin** (~1300 L total), 2 decorative SVG motifs, redundant headers (Messages has THREE), heavy permission prose.
...
- **Delete the 3 blue comms TUI twins + Companion TUI** (~1500 lines off-brand blue).
...
- **MERGE Contacts + Phone + Messages → one unified Comms surface** (people/threads list, tap→timeline, inline call/text, keypad behind one tap, composing/dialing/lookup via chat). Eliminates ~3900 lines + 3 TUI twins. Highest-value merge.
