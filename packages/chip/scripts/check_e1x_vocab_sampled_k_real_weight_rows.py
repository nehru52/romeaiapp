...
REPORT = ROOT / "build/reports/e1x_vocab_sampled_k_real_weight_rows.json"
...
def main() -> int:
    ...
    paths = (PLACEMENT, FULL_OUTPUT_WORKPLAN, FULL_OUTPUT_CHECKSUM_MANIFEST, FULL_NORM_REAL_WEIGHT)
    ...
    deps_ok = (
        ...
        and full_norm.get("summary", {}).get("residual_blocker")
    ...
    target_layers = [
        ...
        if isinstance(layer, dict) and str(layer.get("kind")) in {"embedding", "lm_head"}
    ]
    layer_results: list[dict[str, object]] = []
    total_rows = 0
    total_macs = 0
    full_k_macs_represented = 0
    ...
    represented_mac_fraction = full_k_macs_represented / full_macs if full_macs else 0.0
    execution_ok = (
        len(target_layers) == 2
        and total_rows == 64_000
        and total_macs == 8_192_000
        and full_k_macs_represented == 327_680_000
        and 0.02 < row_fraction < 0.03
        and 0.0006 < executed_mac_fraction < 0.0007
        and 0.02 < represented_mac_fraction < 0.03
    ...
    summary = {
        ...
        "executed_layer_kinds": ["embedding", "lm_head"],
        "executed_layer_count": len(target_layers),
        "sampled_k": SAMPLED_K,
        "executed_vocab_output_row_count": total_rows,
        "executed_vocab_sampled_k_mac_count": total_macs,
        "represented_vocab_full_k_mac_count": full_k_macs_represented,
        ...
        "vocab_sampled_k_real_weight_checksum": int(aggregate_checksum),
        "vocab_sampled_k_result_sha256": canonical_sha256(layer_results),
    ...
    report: dict[str, object] = {
        ...
        "subsystem": "e1x",
        "claim_boundary": (
            "Deterministic W4A8 real-weight execution for every output row of the "
            "embedding and lm_head singleton layers over a sampled K window. This "
            "proves complete row coverage for those vocab-sized layers at sampled K, "
            "not full-K execution for those layers, not full-output graph execution, "
        ...
        "evidence_paths": [
            ...
            "scripts/check_e1x_vocab_sampled_k_real_weight_rows.py",
        ],
        "checks": checks,
        "summary": summary,
    }
    ...
    if failures:
        print(
            "BLOCKED: E1X vocab sampled-K real-weight rows failed: "
            + ", ".join(c["id"] for c in failures)
        )
        return 1
    print(f"PASS: E1X vocab sampled-K real-weight rows; report {REPORT.relative_to(ROOT)}")
    return 0
