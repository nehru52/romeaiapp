...
REPORT = ROOT / "build/reports/e1x_stratified_full_k_repair_execution.json"
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "gate": "e1x-stratified-full-k-repair-execution",
        "status": "PASS" if not failures else "BLOCKED",
        "as_of": datetime.now(UTC).isoformat(),
        "generated_utc": utc_now(),
        "subsystem": "e1x",
        "claim_boundary": (
            "Repair-aware deterministic W4A8 execution for the 16-row-per-layer "
        ...
        "evidence_paths": [
            ...
            "benchmarks/results/e1x-real-graph-model-load.high_failure_defect_map.json",
            "benchmarks/results/e1x-real-graph-model-load.high_failure_repair_manifest.json",
            "scripts/check_e1x_stratified_full_k_repair_execution.py",
        ],
        "checks": checks,
    ...
    if failures:
        print(
            "BLOCKED: E1X stratified full-K repair execution failed: "
            + ", ".join(c["id"] for c in failures)
        )
        return 1
    print(f"PASS: E1X stratified full-K repair execution; report {REPORT.relative_to(ROOT)}")
    return 0
