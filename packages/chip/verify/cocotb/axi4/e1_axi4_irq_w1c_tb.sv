...
// e1_axi4_irq_w1c_tb
//
// Cocotb harness for the write-1-to-clear IRQ status path on
// e1_axi4_interconnect.  The bench wires a single master to a DRAM model
// at offset 0 plus a deliberately unmapped aperture so that decode-error
// edges can be generated on demand.  irq_status_clear_we and the per-IRQ
// clear masks are exposed directly so the cocotb test can model a
// firmware writer to the W1C MMR documented in
// docs/spec-db/axi4-interconnect-mmio.yaml.

module e1_axi4_irq_w1c_tb #(
    ...
    e1_axi4_interconnect #(
        ...
        .irq_status_clear_we              (irq_status_clear_we),
        .irq_status_decode_err_clear_mask (irq_status_decode_err_clear_mask),
        .irq_status_excl_fail_clear_mask  (irq_status_excl_fail_clear_mask)
    );
