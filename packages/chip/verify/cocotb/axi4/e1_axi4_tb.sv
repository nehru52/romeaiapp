...
// e1_axi4_tb
//
// Synthesizable harness wrapping e1_axi4_interconnect + e1_axi4_dram_model
// for cocotb verification of burst, ID ordering, exclusive monitor, write
// strobes, and decode error.  Two upstream masters (NPU + DMA) target a
// single slave port (DRAM model) plus a synthetic "no slave" address range
// that the interconnect promotes to RESP_DECERR.
...
module e1_axi4_tb #(
    ...
    // Tie off unused slave ports so the interconnect never selects them.
    // Slaves 1..NUM_SLAVES-1 are mapped at UNMAP_BASE / UNMAP_MASK which
    // the decode function does not match, so the interconnect routes any
    // out-of-range request to DECERR instead of those slaves.
    generate
