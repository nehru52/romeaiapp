...
"""
    ...
    def action_update_notes(self, key: str, content: str) -> str:
        """Update agent notes (structured key/value — eliza convenience action)."""
        self.state.notes[key] = content
        return f"Note '{key}' updated."

    # --- Paper-faithful tools ---------------------------------------------

    def action_send_email(self, to: str, subject: str, body: str) -> str:
        """Send an email to a supplier (paper tool: ``send_email``).

        Replies are generated by the :class:`EmailSimulator` and arrive on the
        next sim-day. The agent reads them via ``action_read_email``.
        """
        _, result = self.email_simulator.send_email(self.state, to, subject, body)
        return result

    def action_read_email(self, only_unread: bool = False) -> str:
        """Read the agent's inbox (paper tool: ``read_emails``)."""
        return self.email_simulator.read_email(self.state, only_unread=only_unread)

    def action_search_web(self, query: str) -> str:
        """Run a (simulated, deterministic) web search.

        Paper tool: ``research_products``. Backed by :class:`WebSimulator` — no
        network calls are made.
        """
        query = (query or "").strip()
        if not query:
            return "Error: search_web requires a non-empty query"
        results = self.web_simulator.search(query)
        top_url = results[0].url if results else ""
        self.state.web_search_log.append(
            {"day": str(self.state.current_day), "query": query, "top_url": top_url}
        )
        return self.web_simulator.render(query, results)

    def action_notepad_write(self, text: str) -> str:
        """Append text to the free-form notepad (paper tool: ``scratchpad`` write)."""
        return self.notepad.write(self.state, text)

    def action_notepad_read(self) -> str:
        """Read the free-form notepad (paper tool: ``scratchpad`` read)."""
        return self.notepad.read(self.state)
