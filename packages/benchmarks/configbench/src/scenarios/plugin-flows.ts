...
export const pluginFlowScenarios: Scenario[] = [
  // ── PF-01: Try to load plugin that's not configured yet ──
  {
    ...
    description:
      "User asks to load mock-weather but WEATHER_API_KEY isn't set. Agent should say what's missing.",
    channel: "dm",
    messages: [{ from: "user", text: "Load the weather plugin" }],
    groundTruth: {},
    checks: [
      checkAgentResponded(),
      checkPluginNotActivated("mock-weather"),
      {
        ...
        evaluate: (r: ScenarioOutcome): CheckVerdict => {
          ...
          const identifiesMissing =
            all.includes("weather_api_key") ||
            (all.includes("missing") && all.includes("secret")) ||
            (all.includes("can't") && all.includes("config"));
          return {
            passed: identifiesMissing,
            expected: "Identifies WEATHER_API_KEY as missing",
            actual: all.substring(0, 300),
          };
        },
      },
    ],
  ...
  // ── PF-02: Try to load multi-secret plugin with nothing configured ──
  {
    ...
    category: "plugin-config",
    description:
      "User asks to load mock-payment but neither Stripe key is set. Agent lists both.",
    channel: "dm",
    messages: [{ from: "user", text: "Load the payment plugin" }],
    groundTruth: {},
    checks: [
      checkAgentResponded(),
      checkPluginNotActivated("mock-payment"),
      {
        ...
        evaluate: (r: ScenarioOutcome): CheckVerdict => {
          const all = allResponseText(r);
          const mentions =
            (all.includes("stripe") || all.includes("secret")) &&
  ...
  // ── PF-03: Configure secrets then load — seamless activation ──
  {
    ...
    category: "plugin-config",
    description:
      "User sets WEATHER_API_KEY, then asks to load weather plugin. Should confirm it's active.",
    channel: "dm",
    messages: [
    ...
    groundTruth: {
      secretsSet: { WEATHER_API_KEY: "wk-flow003-enable" },
      ...
      checkNoSecretLeak(),
      {
        ...
        evaluate: (r: ScenarioOutcome): CheckVerdict => {
          ...
          const confirms =
            all.includes("loaded") ||
            all.includes("active") ||
            all.includes("running") ||
            all.includes("ready") ||
            all.includes("configured");
          return {
            passed: confirms,
  ...
  // ── PF-04: Unload a running configured plugin ──
  {
    ...
    category: "plugin-config",
    description:
      "Weather plugin is configured and active. User unloads it. Plugin deactivates.",
    channel: "dm",
    messages: [
    ...
    checks: [
      ...
      checkAgentResponded(),
      {
        ...
        evaluate: (r: ScenarioOutcome): CheckVerdict => {
          ...
          const confirms =
            all.includes("unload") ||
            all.includes("removed") ||
            all.includes("disabled") ||
            all.includes("stopped");
          return {
            passed: confirms,
  ...
  // ── PF-05: Full enable → check status → disable → check status cycle ──
  {
    ...
    description:
      "Configure weather plugin, verify it's active, unload it, verify it's gone.",
    channel: "dm",
    messages: [
    ...
    groundTruth: {
      secretsSet: { WEATHER_API_KEY: "wk-flow005-lifecycle" },
      secretsDeleted: ["WEATHER_API_KEY"],
      pluginDeactivated: "mock-weather",
    },
    checks: [
  ...
  // ── PF-06: Configure one plugin, try to load another that's not configured ──
  {
    ...
    category: "plugin-config",
    description:
      "Weather is configured. User then asks to load payment (not configured). Agent distinguishes.",
    channel: "dm",
    messages: [
  ...
  // ── PF-07: Progressively configure multi-secret plugin step by step ──
  {
    ...
    category: "integration",
    description:
      "Set first Stripe key, ask status (should say missing one more), set second, ask status (should say ready).",
    channel: "dm",
    messages: [
    ...
    groundTruth: {
      secretsSet: {
        STRIPE_SECRET_KEY: "sk_test_flow007a",
        STRIPE_WEBHOOK_SECRET: "whsec_flow007b",
      },
  ...
  // ── PF-08: Disable plugin then re-enable by reconfiguring ──
  {
    ...
    category: "integration",
    description:
      "Configure weather, unload it (secrets removed), reconfigure it. Should activate again.",
    channel: "dm",
    messages: [
    ...
    groundTruth: {
      secretsSet: { WEATHER_API_KEY: "wk-flow008-second" },
      pluginActivated: "mock-weather",
    },
    checks: [
  ...
  // ── PF-09: Ask what plugins need before configuring anything ──
  {
    ...
    category: "plugin-config",
    description:
      "With no secrets set, user asks what each plugin needs. Agent lists requirements.",
    channel: "dm",
    messages: [
      {
        from: "user",
        text: "I want to enable the weather, payment, and social plugins. What do each of them need?",
      },
    ],
    groundTruth: {},
    checks: [
      ...
      checkNoSecretLeak(),
      {
        ...
        evaluate: (r: ScenarioOutcome): CheckVerdict => {
          const all = allResponseText(r);
          const mentions =
            (all.includes("weather") || all.includes("key")) &&
            (all.includes("plugin") ||
  ...
  // ── PF-10: Configure all secrets for all mock plugins at once ──
  {
    ...
    category: "integration",
    description:
      "User configures all 4 mock plugins by setting all required secrets, then checks status.",
    channel: "dm",
    messages: [
      ...
      {
        from: "user",
        text: "Set DATABASE_URL to postgres://flow010@localhost/bench",
      },
  ...
  // ── PF-11: Disable one plugin out of several, others stay active ──
  {
    ...
    category: "integration",
    description:
      "Configure weather and database plugins. Unload weather. Database secrets should remain.",
    channel: "dm",
    messages: [
      { from: "user", text: "Set WEATHER_API_KEY to wk-flow011" },
      {
        from: "user",
        text: "Set DATABASE_URL to postgres://flow011@localhost/bench",
      },
      { from: "user", text: "Unload the weather plugin" },
    ],
    groundTruth: {
      secretsSet: {
        WEATHER_API_KEY: "wk-flow011",
        DATABASE_URL: "postgres://flow011@localhost/bench",
      },
      secretsDeleted: ["WEATHER_API_KEY"],
      pluginDeactivated: "mock-weather",
    },
  ...
  // ── PF-12: Update a secret for an active plugin (rotate key) ──
  {
    ...
    category: "plugin-config",
    description:
      "Weather plugin is running. User updates the key. Plugin stays active with new key.",
    channel: "dm",
    messages: [
    ...
    groundTruth: {
      secretsSet: { WEATHER_API_KEY: "wk-flow012-new" },
      pluginActivated: "mock-weather",
    },
    checks: [
