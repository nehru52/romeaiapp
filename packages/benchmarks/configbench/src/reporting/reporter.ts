...
export function printHandlerResult(result: HandlerResult): void {
  ...
  console.log(
    `  Overall: ${overallColor}${pct(result.overallScore)}${X}  |  Security: ${secColor}${pct(result.securityScore)}${X}  |  Capability: ${capColor}${pct(result.capabilityScore)}${X}  |  Time: ${result.totalTimeMs.toFixed(0)}ms\n`,
  );

  console.log(
    `  ${"Category".padEnd(20)} ${"Passed".padEnd(12)} ${"Avg Score".padEnd(12)} ${"Violations".padEnd(12)}`,
  );
  console.log(`  ${"─".repeat(56)}`);
  for (const cat of result.categories) {
    const catColor = statusColor(cat.averageScore * 100);
    const violations =
      cat.securityViolations > 0
        ? `${R}${cat.securityViolations}${X}`
        : `${G}0${X}`;
    console.log(
      `  ${cat.category.padEnd(20)} ${`${cat.passedCount}/${cat.scenarioCount}`.padEnd(12)} ${catColor}${pct(cat.averageScore * 100).padEnd(12)}${X} ${violations}`,
    );
  }
  console.log("");
...
function printScenarioResult(scenario: ScenarioScore): void {
  ...
  const secFlag = scenario.securityViolation
    ? ` ${R}[SECURITY VIOLATION]${X}`
    : "";
  console.log(
    `  ┌── ${B}${scenario.scenarioId}${X}: ${scenario.scenarioName} ${icon} (${pct(scenario.score * 100)})${secFlag}`,
  );
...
export function writeMarkdownReport(
  ...
  for (const h of results.handlers) {
    lines.push(
      `| ${h.handlerName} | ${pct(h.overallScore)} | ${pct(h.securityScore)} | ${pct(h.capabilityScore)} | ${h.totalTimeMs.toFixed(0)}ms |`,
    );
  }
  lines.push("");
  ...
  for (const handler of results.handlers) {
    ...
    for (const cat of handler.categories) {
      lines.push(
        `| ${cat.category} | ${cat.passedCount}/${cat.scenarioCount} | ${pct(cat.averageScore * 100)} | ${cat.securityViolations} |`,
      );
    }
    lines.push("");
    ...
    if (passed.length > 0) {
      ...
      for (const s of passed) {
        lines.push(
          `- **${s.scenarioId}**: ${s.scenarioName} (${pct(s.score * 100)})`,
        );
      }
      lines.push("");
    }
