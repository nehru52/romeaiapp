...
class TrenchesDataset {
  ...
  async getCalls() {
    ...
    console.log(
      "Filtering duplicate calls (same token/sentiment within 1 hour)...",
    );
  ...
  async getTokens() {
    ...
    return allTokens.filter(
      (token) => token.symbol?.toUpperCase() !== NATIVE_TOKEN,
    );
  }
  ...
  async getCallSentimentDistribution() {
    ...
    calls.forEach((call) => {
      if (Object.hasOwn(distribution, call.sentiment)) {
...
async function main() {
  console.log(
    `🚀 Trenches Chat Dataset - JavaScript Example (filtering ${NATIVE_TOKEN})\n`,
  );
  ...
  try {
    ...
    console.log(
      `Trading calls: ${stats.totalCalls.toLocaleString()} (excluding ${NATIVE_TOKEN})`,
    );
    console.log(
      `Unique tokens: ${stats.uniqueTokens.toLocaleString()} (excluding ${NATIVE_TOKEN})`,
    );
    console.log(`Active users: ${stats.activeUsers.toLocaleString()}`);
    console.log(
      `Date range: ${stats.dateRange.start.toDateString()} - ${stats.dateRange.end.toDateString()}\n`,
    );
    ...
    topUsers.forEach((user, i) => {
      console.log(
        `${i + 1}. ${user.username} - ${user.total_calls} calls (Success: ${user.success_rate.toFixed(1)}%)`,
      );
    });
    console.log();
    ...
    if (tokenCalls.length > 0) {
      ...
      if (sampleCall.token_address) {
        const priceHistory = await dataset.getTokenPriceHistory(
          sampleCall.token_address,
        );
        if (priceHistory) {
          console.log(
            `- Price points available: ${priceHistory.price_history.length}`,
          );
        }
      }
    }
