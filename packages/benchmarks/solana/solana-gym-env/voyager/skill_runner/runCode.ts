...
async function runSkill(): Promise<void> {
  const [, , encodedCode, encodedPrograms, agentPubkey, timeoutMsStr] =
    process.argv;

  if (!encodedCode || !encodedPrograms || !agentPubkey || !timeoutMsStr) {
    console.error(
      "Usage: bun runCode.ts <b64Code> <b64Programs> <agentPubkey> <timeoutMs>",
    );
    process.exit(1);
  }
  ...
  // Debug logging
  console.error(
    `DEBUG: Code length: ${code.length}, first 100 chars: ${code.substring(0, 100).replace(/\n/g, "\\n")}`,
  );
  console.error(
    `DEBUG: Programs length: ${programs.length}, first 100 chars: ${programs.substring(0, 100).replace(/\n/g, "\\n")}`,
  );
  ...
  try {
    ...
    if (!serialized_tx) {
      throw new Error(
        "Code evaluation did not return a serialized transaction.",
      );
    }
