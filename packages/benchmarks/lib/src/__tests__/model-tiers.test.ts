...
describe("resolveTier", () => {
  ...
  it("returns the small tier when MODEL_TIER=small", () => {
    ...
    expect(spec.modelName).toBe("qwen3.5-0.8b-q8_0");
    expect(spec.bundlePath).toContain("eliza-1-0_8b.bundle");
  });
  ...
  it("applies MODEL_NAME_OVERRIDE", () => {
    const spec = resolveTier({
      MODEL_TIER: "small",
      MODEL_NAME_OVERRIDE: "qwen3.5-0.8b-q4_k_s",
    });
    expect(spec.modelName).toBe("qwen3.5-0.8b-q4_k_s");
  });
  ...
  it("does not mutate the DEFAULT_TIERS registry on override", () => {
    ...
    expect(DEFAULT_TIERS.small.modelName).toBe("qwen3.5-0.8b-q8_0");
  });
});
