...
class TrajectoryReplayer {
  ...
  async loadTrajectory(key) {
    ...
    try {
      ...
      this.chatArea.innerHTML =
        '<div class="empty-state"><p>Failed to load trajectory.</p></div>';
    }
  }
    ...
    const eventTypes = [
      {
        key: "reset",
        label: "Context Reset",
        cssClass: "event-reset",
        icon: "\u{1F6A8}",
      },
      {
        key: "thinking_reset",
        label: "Thinking Reset",
        cssClass: "event-thinking-reset",
        icon: "\u{1F9E0}",
      },
      {
        key: "trim",
      ...
      if (msg.reasoning_content) {
        ...
        if (hasEncrypted) {
          ...
          reasoningContent = msg.reasoning_details
            .filter(
              (d) => typeof d === "string" || (d.type === "text" && d.text),
            )
  ...
  createMessageElement(msg, _index) {
  ...
  showToolDetails(toolCallId) {
    ...
    // Deselect previous
    document
      .querySelectorAll(".tool-call-chip.selected")
  ...
  resetPlayback() {
    ...
    this.sidebarContent.innerHTML =
      '<p class="sidebar-empty">Click a tool call to view details.</p>';
  ...
  updateMetrics() {
    ...
    if (m.accuracy !== null && m.accuracy !== undefined) {
      ...
      if (m.accuracy >= 0.8)
        this.metricAccuracy.classList.add("metric-accuracy-high");
      else if (m.accuracy >= 0.4)
        this.metricAccuracy.classList.add("metric-accuracy-mid");
  ...
  bindEvents() {
    ...
    this.progressBar.addEventListener("input", () => {
      this.stopPlayback();
      this.goToStep(parseInt(this.progressBar.value, 10));
    });
  ...
  getToolIcon(toolName) {
    ...
    if (name.includes("filesystem") || name.includes("file_system"))
      return "\u{1F4C1}";
    if (name.includes("excel") || name.includes("spreadsheet"))
      return "\u{1F4CA}";
    if (name.includes("canvas")) return "\u{1F393}";
    if (name.includes("google_cloud") || name.includes("bigquery"))
      return "\u2601\uFE0F";
    if (name.includes("snowflake")) return "\u2744\uFE0F";
    if (name.includes("google_sheet")) return "\u{1F4D7}";
    if (name.includes("woocommerce") || name.includes("shop"))
      return "\u{1F6D2}";
