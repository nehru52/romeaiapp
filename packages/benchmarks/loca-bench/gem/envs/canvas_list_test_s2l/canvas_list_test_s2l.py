...
# Constants
TASK_INSTRUCTION = '''This is a synthetic benchmark workspace. Based on the course information available through the provided Canvas tools, workspace files, and benchmark memory records, as well as the synthetic student's assignment and quiz submission status, find all unfinished course assignments and quizzes that have to be completed (find all assignments and quizzes that must be submitted, noting that teacher announcements may say some content does not need to be submitted), organize the information according to the required fields in the workspace's CSV header, keep the format consistent with the examples, and complete these CSV files. Fill the quizzes/assignments in chronological order by their deadlines (DDL), and for quizzes/assignments with the same DDL, sort them in dictionary order of the class code. Directly edit the given 2 CSV files without changing their file names.'''
