...
app.get("/api/late-banner", async (_req, res) => {
  await delay(2500);
  res.json({
    text: "Limited Time Offer: 20% off your first order! Use code WELCOME20",
  });
});

// Side Pane - 1500ms delay
app.get("/api/side-pane", async (_req, res) => {
  await delay(1500);
  res.json({
    categories: [
...
app.get("/api/results-bar", async (_req, res) => {
  await delay(1600);
