      ...
      server.middlewares.use((req, res, next) => {
        ...
        if (pathname === "/api/trajectory") {
          ...
          try {
            ...
            res.end(
              JSON.stringify({
                error: "Failed to read trajectory",
                details: String(e),
              }),
            );
          }
        ...
        if (pathname === "/api/episode") {
          ...
          if (!trialPath || episode === null) {
            ...
            res.setHeader("Content-Type", "application/json");
            res.end(
              JSON.stringify({ error: "Missing path or episode parameter" }),
            );
            return;
          }
        ...
        if (pathname === "/api/file") {
          ...
          if (!trialPath || !filePath) {
            ...
            res.setHeader("Content-Type", "application/json");
            res.end(
              JSON.stringify({ error: "Missing path or file parameter" }),
            );
            return;
          }
          ...
          try {
            ...
            if (stat.isDirectory()) {
              ...
              res.setHeader("Content-Type", "application/json");
              res.end(
                JSON.stringify({ error: "Path is a directory, not a file" }),
              );
              return;
            }
            ...
            if (stat.size > 5 * 1024 * 1024) {
              ...
              res.setHeader("Content-Type", "application/json");
              res.end(
                JSON.stringify({ error: "File too large", size: stat.size }),
              );
              return;
            }
            ...
            res.end(
              isJson ? content : JSON.stringify({ content, size: stat.size }),
            );
          } catch (e) {
            res.statusCode = 500;
            res.setHeader("Content-Type", "application/json");
