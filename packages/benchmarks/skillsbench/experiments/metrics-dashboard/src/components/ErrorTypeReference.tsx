...
export function ErrorTypeReference({
  trials,
  onErrorTypeSelect,
}: ErrorTypeReferenceProps) {
      ...
      <CardContent className="p-4 space-y-4 max-h-[600px] overflow-y-auto">
              ...
              <div className="space-y-1 ml-4">
                {errors.map((error) => (
                  <button
                    key={error.name}
                    onClick={() =>
                      error.count > 0 && onErrorTypeSelect?.(error.name)
                    }
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span
                        className={`text-sm ${error.count > 0 ? "" : "text-muted-foreground"}`}
                      >
                        {error.name}
                      </span>
                      {error.count > 0 ? (
        ...
        {groupedErrors.unknown?.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              <div
                className={`w-2 h-2 rounded-full ${categoryColors.unknown}`}
              />
