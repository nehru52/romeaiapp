"""Mind2Web agent_fn factory backed by the OpenClaw CLI.
...
as ``{operation, element_id, value, reasoning}``. This adapter exposes a
factory that wraps :class:`OpenClawClient` and parses the assistant's JSON
response into that shape.

The runner owns task iteration and scoring; this module is the thin
per-step bridge — symmetric to ``build_bfcl_agent_fn`` but with a
Mind2Web-shaped output dict instead of a tool-call list.
"""
...
def _extract_action_json(text: str) -> dict[str, Any]:
    ...
    Tolerates ``{...}`` directly, ```` ```json ... ``` ```` fences, and
    embedded objects inside conversational prose.
    """
    if not text:
        return {}
    stripped = text.strip()
...
def _coerce_action(
    ...
    """Merge params-channel and text-channel data into the action shape."""
...
def build_mind2web_agent_fn(
    *,
    ...
    ``step_context`` is forwarded to the OpenClaw client unchanged so the
    underlying agent sees the full step bundle (task, candidates, html,
    etc.). The returned dict shape::
    ...
    async def _agent_fn(
        ...
        try:
            ...
            logger.exception("[openclaw-mind2web] send_message failed")
            raise RuntimeError("OpenClaw Mind2Web send_message failed") from exc
