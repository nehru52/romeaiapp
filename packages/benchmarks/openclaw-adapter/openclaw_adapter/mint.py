"""MINT agent_fn factory backed by the OpenClaw CLI.
...
This adapter wraps :class:`OpenClawClient` and threads the conversation
history through ``send_message(text, context={"messages": ...})``. The
returned dict matches the MINT runner's expected per-turn shape: ``text``,
``tool_calls``, and ``thought`` for the assistant turn.
"""
    ...
    *,
    ...
    async def _agent_fn(
        ...
        try:
            ...
            logger.exception("[openclaw-mint] send_message failed")
            raise RuntimeError("OpenClaw MINT send_message failed") from exc
