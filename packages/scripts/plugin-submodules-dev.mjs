 ...
 *     4. Run `packages/scripts/fix-workspace-deps.mjs` when `--check` fails (otherwise skip)
 *
 * Idempotent: safe to run repeatedly; skips submodule update, workspace/self-dep edits, and fix-deps when already satisfied.
 * Writes `.eliza/plugin-dev-needs-install` when anything changed so `packages/scripts/dev.mjs` can run `bun install` only then.
 *
 *   RESTORE (--restore):
 *     1. Run `packages/scripts/fix-workspace-deps.mjs --restore` (parent-repo package.json only)
 ...
 *   - `packages/scripts/replace-workspace-versions.js` for publish (Lerna)
 *
 * Usage:
 *   bun run dev                    # root dev.mjs runs this, then agent watch
 *   node packages/scripts/plugin-submodules-dev.mjs
 *   node packages/scripts/plugin-submodules-dev.mjs --restore
 *   node packages/scripts/plugin-submodules-dev.mjs --quiet
 */
