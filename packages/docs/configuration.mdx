...
`ELIZA_DEFER_LOCAL_EMBEDDING_WARMUP=1` defers startup GGUF embedding prefetch
until the dev/runtime server is ready, then warms in the background.
`ELIZA_SKIP_LOCAL_EMBEDDING_WARMUP=1` skips the prefetch entirely.
`ELIZA_DISABLE_LOCAL_EMBEDDINGS=1` disables local `TEXT_EMBEDDING`
registration entirely. Desktop dev defaults to deferred background warmup;
CI defaults to skip. Set `ELIZA_ENABLE_STARTUP_LOCAL_EMBEDDING_WARMUP=1` to
start warmup during runtime bootstrap when no skip/defer override is present.
