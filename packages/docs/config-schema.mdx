...
directory (default: `~/.local/state/eliza/eliza.json`). All sections are optional; Eliza
...
| [`audio`](#audio) | `AudioConfig` | Open-ended audio settings for subsystem-specific options |
...
Audio configuration. Accepts arbitrary key-value pairs for audio subsystem options; typed fields will be added as stable audio settings are promoted into the shared config schema.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `dataDir` | `string` | `~/.local/state/eliza/workspace/.elizadb` | PGLite data directory |
