...
export function transformTokenPerformance(
	...
	const input = {
		...
		createdAt: dbToken.created_at
			? new Date(dbToken.created_at as string | number)
			: new Date(),
		updatedAt: dbToken.updated_at
			? new Date(dbToken.updated_at as string | number)
			: new Date(),
	};
...
export function transformTokenRecommendation(
	...
	try {
		return tokenRecommendationSchema.parse({
			...
			createdAt: new Date(
				(dbRec.createdAt || dbRec.created_at || Date.now()) as string | number,
			),
			updatedAt: new Date(
				(dbRec.updatedAt || dbRec.updated_at || Date.now()) as string | number,
			),
		});
	} catch (error) {
