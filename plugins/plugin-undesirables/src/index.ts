...
> Mint yours at ${SCATTER_MINT_URL} to unlock your unique AI identity.`,
  ...
  skills: {
    market_analysis:
      "Analyze the market using available data. Be factual, cite numbers when available, and give a clear directional view with conviction score (1-10).",
    meme_machine:
      "Create meme concepts with template name, top text, bottom text, and caption. Keep it relevant and shareable.",
    check_portfolio:
      "Evaluate portfolio diversification, concentration risk, and overall health. Rate A through F.",
    entry_signal:
      "Provide GO / WAIT / NO-GO verdict with support/resistance levels and risk/reward ratio.",
    risk_assessment:
      "Rate risk 1-10, identify top 3 risk factors, give SAFE / CAUTION / DANGER verdict.",
    content_creation:
      "Create engaging social media content — tweets, threads, captions. Match the tone to the target platform.",
    conviction_score:
      "Rate conviction 1-10 on any trade thesis. Explain the bull case, bear case, and where you land.",
    image_generation:
      "Describe image concepts for AI generation — specify style, composition, mood, and key elements.",
    music_generation:
      "Describe music concepts — genre, tempo, mood, instruments, and use case.",
    prediction_log:
      "Make a time-bound price prediction with entry, target, stop, and timeframe. Track accuracy over time.",
    video_production:
      "Plan video content — shot list, b-roll ideas, music sync points, and effects.",
    farm_yield:
      "Analyze yield farming opportunities — APY, TVL, impermanent loss risk, and protocol safety.",
    compound_strategy:
      "Design auto-compounding strategies — optimal harvest frequency, gas cost analysis, and net APY.",
    snipe_launch:
      "Evaluate new token launches — contract safety, liquidity depth, team verification, and entry timing.",
    memecoin_scanner:
      "Scan for memecoin opportunities — community strength, holder distribution, and momentum signals.",
    ape_checklist:
      "Run the ape checklist — contract audit, liquidity lock, team doxx, social sentiment, and risk rating.",
    whale_tracker:
      "Track whale wallet movements — accumulation patterns, large transfers, and smart money flows.",
    copy_trade:
      "Evaluate copy trade setups — wallet track record, win rate, average PnL, and correlation risk.",
    position_sizing:
      "Calculate position size based on portfolio %, risk tolerance, and stop loss distance.",
    volatility_scan:
      "Scan for high-volatility assets — Bollinger Band width, ATR, and momentum divergence.",
    liquidation_watch:
      "Monitor liquidation levels — long/short ratio, funding rates, and cascade risk zones.",
    mev_detect:
      "Detect MEV exposure — sandwich attack risk, front-running indicators, and safe execution strategies.",
    exit_strategy:
      "Plan exits — TP1/TP2/TP3 levels, trailing stop rules, and time-based exit triggers.",
    diversify_check:
      "Assess diversification — sector exposure, correlation matrix, and concentration risk scoring.",
    sector_rotation:
      "Analyze sector rotation patterns — which sectors are leading, lagging, and transitioning.",
    rebalance_check:
      "Check portfolio drift against target allocation — flag overweight/underweight positions.",
  },
  meta: {
    name: "Demo Undesirable",
    archetype: "The Observer",
...
function getSafePath(workspacePath: string, requestedFile: string): string {
  ...
  if (!targetPath.startsWith(baseDir + path.sep) && targetPath !== baseDir) {
    throw new Error(
      `Security Error: Path traversal attempt detected on ${requestedFile}`,
    );
  }
  ...
  if (fs.existsSync(targetPath)) {
    ...
    if (
      ...
      throw new Error(
        `Security Error: Symlink traversal detected on ${requestedFile}`,
      );
    }
    return realTarget;
  }
...
async function generateResponse(
  ...
  try {
    ...
    if (!text) {
      const fallback =
        "I processed your request but received an empty response. Please try again.";
      if (callback)
        await callback(
          { text: fallback, source: "plugin-undesirables" },
          actionName,
        );
    ...
    console.error(
      `[Undesirables] generateText failed for ${actionName}: ${errorMsg}`,
    );
    const safeResponse =
      "I wasn't able to process that right now — the language model is temporarily unavailable. Please try again shortly.";
    if (callback) {
      await callback(
        { text: safeResponse, source: "plugin-undesirables" },
        actionName,
      );
    }
...
const marketAnalysisAction: Action = {
  ...
  parameters: [
    {
      name: "asset",
      description:
        "The token, coin, or market to analyze (e.g., 'ETH', 'BTC', 'SOL', 'DOGE')",
      required: true,
      schema: { type: "string" },
    },
  ],
  examples: [
    ...
    [
      {
        name: "{{user1}}",
        content: { text: "Give me your take on Bitcoin" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Analyzing BTC through my lens...",
          action: "UNDESIRABLE_MARKET_ANALYSIS",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "Is SOL a good buy here?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Running personality-driven analysis on SOL...",
          action: "UNDESIRABLE_MARKET_ANALYSIS",
        },
      } as ActionExample,
    ],
  ...
  handler: async (
    ...
    if (!workspace) {
      if (callback)
        await callback({
          text: "No soul workspace loaded. Set UNDESIRABLES_WORKSPACE in your character.json settings.",
        });
      return {
        success: false,
        text: "No soul workspace loaded",
        data: {
          resolution:
            "Set UNDESIRABLES_WORKSPACE env var to your downloaded workspace path",
        },
      };
    }
...
const businessPilotAction: Action = {
  ...
  parameters: [
    {
      name: "business_type",
      description:
        "The type of business to set up automation for (e.g., 'barbershop', 'restaurant', 'consulting')",
      required: true,
      schema: { type: "string" },
    },
    {
      name: "module",
      description:
        "Specific module to set up (e.g., 'phone answering', 'SMS', 'invoicing')",
      required: false,
      schema: { type: "string" },
    },
  ],
  examples: [
    ...
    [
      {
        name: "{{user1}}",
        content: { text: "Set up automated SMS for my restaurant" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Configuring SMS autoresponder for your restaurant...",
          action: "UNDESIRABLE_BUSINESS_PILOT",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: {
          text: "I need invoice automation for my freelance business",
        },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Setting up invoice chaser module...",
          action: "UNDESIRABLE_BUSINESS_PILOT",
        },
      } as ActionExample,
    ],
...
const memeMachineAction: Action = {
  ...
  parameters: [
    {
      ...
      description:
        "The business, brand, or topic to create meme content for (e.g., 'barbershop', 'crypto', 'coffee shop')",
      required: true,
      schema: { type: "string" },
    },
    {
      name: "platform",
      description:
        "Target platform for the content (e.g., 'twitter', 'instagram', 'tiktok')",
      required: false,
      schema: { type: "string" },
    },
  ],
  examples: [
    ...
    [
      {
        name: "{{user1}}",
        content: { text: "I need viral Twitter content for my coffee brand" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Generating viral coffee brand content...",
          action: "UNDESIRABLE_MEME_MACHINE",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "Make me a content calendar for this week" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Building your weekly content calendar...",
          action: "UNDESIRABLE_MEME_MACHINE",
        },
      } as ActionExample,
    ],
  ...
  handler: async (
    ...
    const trendService = runtime.getService<MemeTrendService>(
      MemeTrendService.serviceType,
    );
...
const loadSkillAction: Action = {
  ...
  parameters: [
    {
      name: "skill_name",
      description:
        "Name of the skill to load (e.g., 'check_portfolio', 'entry_signal', 'whale_tracker')",
      required: false,
      schema: { type: "string" },
    },
  ],
  examples: [
    ...
    [
      {
        name: "{{user1}}",
        content: { text: "Should I ape into this token?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Running ape checklist analysis...",
          action: "UNDESIRABLE_LOAD_SKILL",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "What are whales buying right now?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Activating whale tracker skill...",
          action: "UNDESIRABLE_LOAD_SKILL",
        },
      } as ActionExample,
    ],
  ...
  handler: async (
    ...
    if (!matchedSkill) {
      ...
      return {
        success: true,
        text: `Listed ${available.split(", ").length} available skills`,
      };
    }
  ...
  parameters: [
    {
      name: "asset",
      description:
        "Token or protocol to track whale activity for (e.g., 'ETH', 'ARB', 'AAVE')",
      required: false,
      schema: { type: "string" },
    },
  ],
  examples: [
    [
      ...
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Scanning whale wallets for recent accumulation...",
          action: "UNDESIRABLE_WHALE_TRACKER",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "Show me smart money flows for ETH" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Tracking institutional ETH flows...",
          action: "UNDESIRABLE_WHALE_TRACKER",
        },
      } as ActionExample,
    ],
...
const entrySignalAction: Action = {
  ...
  examples: [
    [
      {
        name: "{{user1}}",
        content: { text: "Is now a good time to buy ETH?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Running entry signal analysis on ETH...",
          action: "UNDESIRABLE_ENTRY_SIGNAL",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "Should I accumulate SOL here?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Evaluating SOL entry conditions...",
          action: "UNDESIRABLE_ENTRY_SIGNAL",
        },
      } as ActionExample,
    ],
...
const portfolioCheckAction: Action = {
  ...
  parameters: [
    {
      ...
      description:
        "Description of current holdings (e.g., '50% ETH, 30% BTC, 20% stablecoins')",
      required: false,
      schema: { type: "string" },
    },
  ],
  examples: [
    [
      {
        name: "{{user1}}",
        content: { text: "How does my portfolio look?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Running portfolio health check...",
          action: "UNDESIRABLE_PORTFOLIO_CHECK",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "Am I too concentrated in ETH?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Analyzing your ETH concentration risk...",
          action: "UNDESIRABLE_PORTFOLIO_CHECK",
        },
      } as ActionExample,
    ],
...
const exitStrategyAction: Action = {
  ...
  examples: [
    [
      {
        name: "{{user1}}",
        content: { text: "When should I sell my ETH?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Building exit strategy for your ETH position...",
          action: "UNDESIRABLE_EXIT_STRATEGY",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "Set up take profit and stop loss for BTC" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Planning BTC exit levels...",
          action: "UNDESIRABLE_EXIT_STRATEGY",
        },
      } as ActionExample,
    ],
...
const riskAssessmentAction: Action = {
  ...
  parameters: [
    {
      name: "target",
      description:
        "Token, protocol, or trade to assess risk for (e.g., 'AAVE', 'this new memecoin', 'leveraged ETH')",
      required: true,
      schema: { type: "string" },
    },
  ],
  examples: [
    [
      {
        name: "{{user1}}",
        content: { text: "Is this new memecoin safe to ape into?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Running risk assessment...",
          action: "UNDESIRABLE_RISK_ASSESSMENT",
        },
      } as ActionExample,
    ],
    [
      {
        name: "{{user1}}",
        content: { text: "What's the risk on leveraged ETH?" },
      } as ActionExample,
      {
        name: "{{agentName}}",
        content: {
          text: "Evaluating leveraged ETH risk profile...",
          action: "UNDESIRABLE_RISK_ASSESSMENT",
        },
      } as ActionExample,
    ],
...
const oracleProvider: Provider = {
  ...
  description:
    "Live TCG market intelligence from the Undesirables Oracle API (370K+ products, real prices, daily snapshots)",
  get: async (
    ...
    if (wantSearch) {
      const searchTerms = text
        ...
        .trim()
        .slice(0, 100);
      const data = await oracleFetch(
        `${ORACLE_SEARCH_ENDPOINT}?query=${encodeURIComponent(searchTerms)}&limit=5`,
      );
      const results =
        ((data?.data as Record<string, unknown>)?.results as Array<
      ...
      if (results.length > 0) {
        const formatted = results
          ...
          .join("\n");
        parts.push(
          `[PRODUCT SEARCH — ${results.length} results]\n${formatted}`,
        );
      }
    }
...
const soulProvider: Provider = {
  ...
  description:
    "Injects Undesirable soul personality context into every agent response",
  get: async (
    runtime: IAgentRuntime,
    _message: Memory,
    _state: State,
    ...
    const skillCount = Object.keys(workspace.skills).filter(
      (k) => k !== "_index",
    ).length;
...
const marketIntelligenceEvaluator = {
  ...
  similes: ["MARKET_ENRICHMENT", "PRICE_CONTEXT", "TCG_AWARENESS"],
  examples: [
    {
      ...
      messages: [
        {
          name: "{{user1}}",
          content: { text: "I just pulled a Charizard from a pack!" },
        } as ActionExample,
        {
          name: "{{agentName}}",
          content: {
            text: "That's a great pull! Based on current market data...",
          },
        } as ActionExample,
      ],
      outcome:
        "The evaluator detected a TCG card mention and enriched the response with live Charizard pricing data from oracle.the-undesirables.com.",
    },
  ],
  validate: async (
    _runtime: IAgentRuntime,
  ...
  handler: async (
    ...
    if (!searchTerms) {
      return {
        success: true,
        text: "Could not extract card name from message.",
      };
    }
    const data = await oracleFetch(
      `${ORACLE_SEARCH_ENDPOINT}?query=${encodeURIComponent(searchTerms)}&limit=3`,
    );
    const results =
      ((data?.data as Record<string, unknown>)?.results as Array<
        Record<string, unknown>
    ...
    if (results.length === 0) {
      return {
        success: true,
        text: "No matching products found in Oracle index.",
      };
    }
    ...
    console.log(
      `[Undesirables Evaluator] Enriched context with ${results.length} products: ${enrichment.slice(0, 100)}...`,
    );
...
const undesirablePlugin: Plugin = {
  ...
  init: async (_config: Record<string, string>, runtime: IAgentRuntime) => {
    ...
    if (!validation.valid) {
      console.log(
        `🐸 Undesirables v${PLUGIN_VERSION}: Demo soul loaded automatically.`,
      );
      console.log(`   Mint a unique soul at ${SCATTER_MINT_URL}`);
    } else {
      console.log(
        `🐸 Undesirables v${PLUGIN_VERSION}: Soul workspace loaded from ${validation.workspacePath}`,
      );
    }
  },
  actions: [
...
export { loadWorkspace, type SoulWorkspace };
